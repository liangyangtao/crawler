...
/****
 * 
 * @Title: PublicStaffJobParser.java
 * @Package com.kf.data.pdfparser.parser
 * @Description: 公转书 员工专业或岗位
 * @author liangyt
 * @date 2017年10月24日 下午5:16:46
 * @version V1.0
 */
public class PublicStaffJobParser extends PublicBaseParser {
		...
		try {
			...
			for (int j = 1; j < trElements.size(); j++) {
				...
				for (int k = 0; k < tdElements.size(); k++) {
					...
					if (key.contains("类别") || key.contains("专业") || key.contains("结构") || key.contains("岗位")
							|| key.contains("职能")) {
						property = "category";
		...
		for (int j = 0; j < elements.size(); j++) {
			...
			if (childElement.tagName().equals("table")) {
				...
				if (isFind) {
					...
					firstTrText = firstTrText.replace(" ", "");
					firstTrText = firstTrText.replace("&nbsp;", "");
					if ((firstTrText.contains("类别") || firstTrText.contains("专业") || firstTrText.contains("结构")
							|| firstTrText.contains("岗位") || firstTrText.contains("职能")) && firstTrText.contains("人数")
							&& (firstTrText.contains("占比") || firstTrText.contains("比例"))) {
						if (j - 1 > 0) {
							Element preElement = elements.get(j - 1);
							if (preElement.text().contains("岗位结构") || preElement.text().contains("专业结构")
									|| preElement.text().contains("管理职能") || preElement.text().contains("岗位类型")
									|| preElement.text().contains("岗位划分")) {
								...
								if (j - 2 > 0) {
									...
									if (preElement2.text().contains("岗位结构") || preElement2.text().contains("专业结构")
											|| preElement2.text().contains("管理职能")
											|| preElement2.text().contains("岗位类型")
											|| preElement2.text().contains("岗位划分")) {
										result.add(childElement);
										Elements trElements = childElement.select("tr");
										// 是表头的那个表
										for (Element trElement : trElements) {
