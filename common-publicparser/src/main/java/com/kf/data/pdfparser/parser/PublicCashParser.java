 ...
 * @version V1.0
 */
public class PublicCashParser extends PublicBaseParser {
	...
	private List<Element> fillResult(Elements elements) {
		...
		for (int j = 0; j < elements.size(); j++) {
			...
			if (isFind) {
				if (childElement.text().contains("权益变动表")) {
					break;
				}
			}
				...
				if (isFind) {
					...
					if (firstTrText.contains("年")) {
						if (j - 1 > 0) {
							...
							if (preElement.text().contains("合并现金流量表")) {
								result.add(childElement);
								Elements trElements = childElement.select("tr");
								// 是表头的那个表
								for (Element trElement : trElements) {
								...
								if (j - 2 > 0) {
									...
									if (preElement2.text().contains("合并现金流量表")) {
										result.add(childElement);
										Elements trElements = childElement.select("tr");
										// 是表头的那个表
										for (Element trElement : trElements) {
