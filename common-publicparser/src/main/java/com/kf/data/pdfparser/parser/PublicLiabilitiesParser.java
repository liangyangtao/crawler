 ...
 * @version V1.0
 */
public class PublicLiabilitiesParser extends PublicBaseParser {
	...
	private List<Element> fillResult(Elements elements) {
		...
		for (int j = 0; j < elements.size(); j++) {
			...
			if (isFind) {
				if (childElement.text().contains("合并利润表")) {
					break;
				}
			}
			if (childElement.tagName().equals("table")) {
				...
				if (isFind) {
					...
					firstTrText = firstTrText.replace("	", "");
					firstTrText = firstTrText.replace(" ", "");
					if (firstTrText.contains("年") || firstTrText.contains("月") || firstTrText.contains("日")) {
						if (j - 1 > 0) {
							Element preElement = elements.get(j - 1);
							if (preElement.text().contains("合并资产负债表")) {
								result.add(childElement);
								Elements trElements = childElement.select("tr");
								// 是表头的那个表
								for (Element trElement : trElements) {
								...
								if (j - 2 > 0) {
									...
									if (preElement2.text().contains("合并资产负债表")) {
										result.add(childElement);
										Elements trElements = childElement.select("tr");
										// 是表头的那个表
										for (Element trElement : trElements) {
