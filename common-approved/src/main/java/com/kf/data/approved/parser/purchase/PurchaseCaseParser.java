...
/****
 * @Title: PurchaseCaseParser.java
 * @Package com.kf.data.approved.parser.purchase
 * @Description: 现金
 * @author liangyt
 * @date 2018年1月10日 上午11:09:29
 * @version V1.0
 */
public class PurchaseCaseParser extends BaseParser {
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getCaseResult(Document document) {
		List<Map<String, Object>> resultMap = new ArrayList<>();
		try {
			Elements pElements = document.select("div").first().children();
			List<Element> result = fillCaseResult(pElements);
			if (result == null) {
				return resultMap;
			}
			if (result.size() == 0) {
	 ...
	 * @return
	 */
	public List<Element> fillCaseResult(Elements elements) {
		...
		for (int j = 0; j < elements.size(); j++) {
			...
			if (isFind) {
				if (childElement.text().contains("权益变动表") || childElement.text().contains("负债表 ")
						|| childElement.text().contains("利润表 ")) {
					break;
				}
			}
