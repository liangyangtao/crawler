...
/****
 * 
 * @Title: PurchaseProfitParser.java
 * @Package com.kf.data.approved.parser.purchase
 * @Description: 利润表
 * @author liangyt
 * @date 2018年1月10日 上午11:38:16
 * @version V1.0
 */

public class PurchaseProfitParser extends BaseParser {
	/****
	 * 解析利润表
	 * 
	 * @param pdfCodeTable
	 ...
	 * @return
	 */
	public List<Map<String, Object>> getProfitResult(Document document) {
		List<Map<String, Object>> resultMap = new ArrayList<>();
		try {
			Elements pElements = document.select("div").first().children();
			List<Element> result = fillProfitResult(pElements);
			if (result == null) {
				return resultMap;
			}
			if (result.size() == 0) {
	 ...
	 * @return
	 */
	public List<Element> fillProfitResult(Elements elements) {
